...
const StyledSelectTrigger = styled('div')<{ isOpen: boolean; hasError?: boolean }>`
  ...
  border: 1px solid
    ${({ theme, hasError }) =>
      hasError ? theme.palette.error.main : theme.palette.customColors.light};
  border-radius: 13px;
  cursor: pointer;
...
const CustomSelect: React.FC<ICustomSelectProps> & { Option: typeof Option } = ({
  labelName,
  ...
  // Lock body scroll when open (via event prevention to keep scrollbar + thumb intact)
  useEffect(() => {
    if (!isOpen) return;

    const preventWheel = (e: WheelEvent) => {
      const dropdown = dropdownRef.current;
      // If scrolling inside the dropdown menu, clamp at boundaries
